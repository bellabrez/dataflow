...
def main(args):
	...
	behaviors = ['Y', 'Z_pos', 'Z_neg', 'W']

	#########################################################
	### Pool behavior to calculate std across the dataset ###
	#########################################################
	# this is the new bit

	pooled_behavior = {}
	for behavior in ['Y', 'Z_pos', 'Z_neg']:
	    pooled_behavior = []
	    for fly in flies:
	    	beh_trace = flies[fly].fictrac.fictrac[behavior.split('_')[0]]
	    	if 'pos' in behavior:
				beh_trace = np.clip(beh_trace a_min=0, a_max=None)
			if 'neg' in behavior:
				beh_trace = np.clip(beh_trace, a_min=None, a_max=0)*-1
	        pooled_behavior.append(beh_trace)
	    pooled_behavior = np.asarray(pooled_behavior).flatten()
	    STD_BEH[behavior] = np.std(behavior_super[behavior])
	    printlog(F'STD_BEH {behavior}, {STD_BEH[behavior]}')
	...
	### Save Responses ###
	######################
	save_file = F"/oak/stanford/groups/trc/data/Brezovec/2P_Imaging/20210316_neural_weighted_behavior/20231204_master_X_threshold_reviewer"
