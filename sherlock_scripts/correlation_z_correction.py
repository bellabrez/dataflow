...
def main(args):
    ...
    for z in range(49):
        fictrac_interp = interp_fictrac(fictrac_raw, fps, resolution, expt_len, timestamps, behavior, z)
        for i in range(256):
            for j in range(128):
    ...
    save_file = os.path.join(corr_directory, '20201104_corr_{}.nii'.format(behavior))
    nib.Nifti1Image(corr_brain, np.eye(4)).to_filename(save_file)
    printlog("Saved {}".format(save_file))
    
def interp_fictrac(fictrac, fps, resolution, expt_len, timestamps, behavior, z):
    ...
    if behavior == 'Y':
        fictrac_smoothed = dy
    if behavior == 'Y_pos':
        fictrac_smoothed = np.clip(dy, a_min=0, a_max=None)
    if behavior == 'Y_neg':
        fictrac_smoothed = np.clip(dy, a_min=None, a_max=0)
    if behavior == 'Z_abs':
