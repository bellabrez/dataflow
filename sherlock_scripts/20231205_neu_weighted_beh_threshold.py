...
def main(args):
	...
	### THRESHOLD USING STD ###
	###########################
	STD_BEH = {'Y': 0.00809, 'Z_pos': 0.0118, 'Z_neg': 0.0152} #this is actually 0.75 std
	X[:,:,:500,:][X[:,:,:500,:]<STD_BEH['Y']] = 0
	X[:,:,500:1000,:][X[:,:,500:1000,:]<STD_BEH['Z_pos']] = 0
	X[:,:,1000:1500,:][X[:,:,1000:1500,:]<STD_BEH['Z_neg']] = 0
	printlog('thresholding DONE!')
	...
	for z in range(9,49-9):
		...
		### Save Responses ###
		######################
		save_file = F"/oak/stanford/groups/trc/data/Brezovec/2P_Imaging/20210316_neural_weighted_behavior/threshold/responses_{z}"
